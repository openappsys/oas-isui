...
**验证环境**：人工/截图审查用 `pnpm dev`（5173，dev 链路自带 watch 构建，改组件源码后等自动 full reload 即可，无需重启/清缓存）；自动化 e2e 用 `pnpm test:e2e`（4173 preview，webServer 自动 build docs + preview，改组件源码后须先 `pnpm --filter @oas-isui/ui build`，否则 preview 用旧 dist）。截图用 playwright。
