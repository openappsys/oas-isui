...
- **menu/dropdown/contextmenu 三组件能力补齐**（业界能力并集）：
...
- **Tabs 能力全量补齐**（能力补齐）：
  ...
  - 溢出 `more`：滚动 + 视口外标签镜像下拉（业界通行机制，标签不隐藏），搜索过滤、点选平滑滚动到可见区（激活项与相邻项一起进入视口）
...
- `@oas-isui/ssr` 渲染器包：`renderToString(tag, attrs, slotHTML, options)` 输出 Declarative Shadow DOM（DSD）静态快照；`@oas-isui/ui` 新增 Node-safe `ssr` 子路径导出（不执行 `customElements.define`、不触碰 DOM API）
...
- `@oas-isui/nuxt`：Nuxt 3 module——`modules: ['@oas-isui/nuxt']` 开箱即用（Vite `vite:extendConfig` 注入 Vue `isCustomElement` 识别 oas-* + `@oas-isui/theme` 自动注入 `nuxt.options.css` + `renderOasToString`/`useOasRender` SSR helper 自动导入）
- `@oas-isui/next`：Next.js App Router 集成——RSC `<OasComponent>`（`@oas-isui/next/server`）服务端产 DSD 快照（dangerouslySetInnerHTML 进 SSR 输出流）+ `<OasRegistry>` "use client" 客户端注册引导 + `renderOas` 纯逻辑包装（attrs 值自动序列化）

### 变更

- select 多选标签默认换行自适应高度，折叠由 `max-tag-count` 显式启用（对齐业界惯例）
...
- 正式发布 `@oas-isui/ui`、`@oas-isui/core`、`@oas-isui/theme`、`@oas-isui/icons` 至 npm
...
- 基础运行时 `@oas-isui/core`、主题 token、图标库
- 新增 Button、Icon、Tag、Badge、Space、Divider、Link、Typography

[1.0.0]: https://github.com/oas-isui/oas-isui/releases/tag/v1.0.0
[1.1.0]: https://github.com/oas-isui/oas-isui/tree/v1.1.0
[1.2.0]: https://github.com/oas-isui/oas-isui/tree/v1.2.0
[1.3.0]: https://github.com/oas-isui/oas-isui/tree/v1.3.0
[1.4.0]: https://github.com/oas-isui/oas-isui/tree/v1.4.0
[1.5.0]: https://github.com/oas-isui/oas-isui/tree/v1.5.0
[1.6.0]: https://github.com/oas-isui/oas-isui/tree/v1.6.0
[1.7.0]: https://github.com/oas-isui/oas-isui/tree/v1.7.0
[1.8.0]: https://github.com/oas-isui/oas-isui/tree/v1.8.0
