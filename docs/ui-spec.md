...
- 文案：组件内置文案（空态"暂无数据"、分页"共 X 条"、confirm"确定/取消"等）一律走 locale registry（`@oas-isui/i18n`），**禁止在组件里硬编码文案**；locale key 全集有类型约束，漏翻译编译期/测试期报错（locale-completeness 测试）
