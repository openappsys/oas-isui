...
| 组件基类 | 自研轻量基类（`OASElement`，基于 `HTMLElement`） | 核心无第三方运行时依赖；若体量失控再评估 Lit |
...
- 每个组件可独立按需引入：`import '@oas-isui/ui/basic/button'`（side-effect 注册）与 `import { defineButton } from '@oas-isui/ui'`（显式注册）双路径
