...
| 组件基类 | 自研轻量基类（`OASElement`，基于 `HTMLElement`） | 核心无第三方运行时依赖；响应式自研（见 §2.1），不引 Lit |
...
两种"响应式"分别自研，不引入 Lit / Vue reactivity / 第三方信号库。

**决策理由**：零运行时依赖是本项目的核心卖点；组件库场景更新粒度小（单组件 shadow 内），自研成本远低于应用级场景。Lit 的响应式虽成熟，但引入即破坏零依赖承诺，且其模板体系（lit-html）会重塑组件写法，沉没成本高。

**风险与对策**：自研 signal 的坑在内存泄漏（effect 未清理）与更新抖动（未批处理）——以测试矩阵兜底（见下"验收"）。

**A. 数据响应式（signal）**

core 提供约百行的最小实现，API 对齐业界 signal 心智：
...
- **主题注入**：组件样式只写 token 引用；宿主只需一次 `import '@oas-isui/theme'` 定义 `:root` 变量即可换肤。
...
- 每个组件可独立按需引入：`import '@oas-isui/ui/basic/button'`（side-effect 注册）与 `import { OASButton } from '@oas-isui/ui'`（显式注册）双路径
