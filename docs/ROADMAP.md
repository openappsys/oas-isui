...
| **v0.10.0** | 国际化：locale registry（`@oas-isui/i18n`，`setLocale()` 全局切换）+ 语言包 zh-CN/en（tree-shakable）+ 全组件内置文案改造为 locale key + locale-completeness 类型约束测试 + Intl.* 数字/日期格式化规矩 + RTL 逻辑 CSS 属性规矩（全量 RTL 视觉审计挪 v1.x） | ✅ 已完成 |
