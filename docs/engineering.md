...
- **业界对比**：`docs/perf-baseline.md` §3（中性口径，参考值一律标「约」）。
...
- 硬性要求（npm 官方）：**仓库托管在 GitHub**（GitHub-hosted runner；当前 origin 是 Gitea，需迁移/镜像到 GitHub）；Node ≥ 22.14 + npm CLI ≥ 11.5.1（workflow 已升级）；各包 `package.json` 的 `repository.url` 必须与 GitHub 仓库精确一致
...
- **granular token（90 天期，应急）**：npm → Access Tokens → Granular（限 `@oas-isui/*` + publish，可勾 Bypass 2FA 免每次 OTP）→ 写入用户级 `~/.npmrc`（**勿提交仓库**），到期重新生成
- 可选最严姿态：包 Settings → Publishing access → "Require 2FA and disallow tokens"，则 token 全废，本地只能交互式、CI 只能 OIDC

## 6. 文档站部署（Cloudflare + oasisui.dev）

文档站是 Vitepress SSG，静态构建产物部署到 Cloudflare（Workers Static Assets，`wrangler deploy`），绑定自定义域名 `oasisui.dev`（域名 DNS 托管到 Cloudflare，自动 HTTPS）。

**双域策略**：`oasisui.dev` 为主域（可发音、可传播，去连字符可读化）；`oas-isui.dev`（与包名逐字符一致）持作 301 跳转域——Cloudflare 后台 → Rules → Redirect Rules，单条规则把 `oas-isui.dev/*` 301 到 `oasisui.dev` 对应路径（保路径跳转，不在 wrangler.jsonc 里声明）。
...
3. 绑域名：Cloudflare 后台 → Workers → 该项目 → Custom Domains → Add `oasisui.dev`

**要点**：

- `docs build` 依赖 `@oas-isui/ui` 的 dist（workspace symlink）——全量 `pnpm build` 拓扑序自动先构建 ui，无需手动指定
