...
## 6. 文档站部署（Cloudflare Pages + oasisui.dev）

文档站是 Vitepress SSG，静态构建产物部署到 Cloudflare Pages，绑定自定义域名 `oasisui.dev`（域名 DNS 托管到 Cloudflare，自动 Let's Encrypt HTTPS）。

**Cloudflare Pages 配置**：

1. Pages → Create project → 连接 Git 仓库（oas-isui）
2. 构建设置：
   - 构建命令：`pnpm install --frozen-lockfile && pnpm --filter @oas-isui/ui build && pnpm --filter @oas-isui/docs build`
   - 输出目录：`packages/docs/docs/.vitepress/dist`
   - Node 版本：环境变量 `NODE_VERSION=22`（或仓库 `.node-version`）
3. Custom domains → Add `oasisui.dev`
4. 自动部署：push main → 自动构建发布；预览分支生成 `<pr>-oas-isui.pages.dev` 独立 URL

**要点**：

- `docs build` 依赖 `@oas-isui/ui` 的 dist（workspace symlink）——必须先 `pnpm --filter @oas-isui/ui build`，已含在构建命令
