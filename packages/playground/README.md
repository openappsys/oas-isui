...
- **消息 API**：`window.OASMessage` 不存在，必须 `import { message } from '@oas-isui/ui'` 后调用 `message.success(...)`。
...
- 两个 vite config 已配 `optimizeDeps.exclude: ['@oas-isui/ui', '@oas-isui/theme']`（linked 包走 dist ESM 直服），ui dist 重建后 dev 立即生效，无需清缓存。
