...
test('table size 密度档位：small/medium/large 三档 padding+字号阶梯，组件级变量覆盖优先', async ({
  ...
  // 设计（业界主流三档密度惯例）：档位全走 CSS 变量 token
...
test('link 色板达标：预设名映射 -text 达标 token、自定义色原值渲染、type 语义色改指 text 变体', async ({
  ...
  // v2.1 link 色板对齐业界（设计期文字 token 模型）：预设亮色（gold 等）白底本色不达标，
...
test('tabs more 模式（业界机制）：tab 全渲染不隐藏 + more 下拉列视口外 tab + 点选平滑滚动到可见区', async ({
  page,
}) => {
  // 业界通行（滚动 + 视口外镜像下拉）：more 不再是 display:none 收缩，而是 tab 全部渲染 +
