...
test.describe('官网首页', () => {
  test('hero 渲染：标题/双 CTA/console 零告警', async ({ page }) => {
    ...
    await expect(page.locator('.VPHero h1')).toContainText('OAS-ISUI')
  ...
  test('组件墙可交互：按钮出 message、switch 翻转、segmented 切换', async ({ page }) => {
    ...
    // ui 包在 theme/index.ts 里懒加载（import('@oas-isui/ui')）后才挂载 window.message；
    ...
    await expect(msg).toContainText('来自 OAS-ISUI 的问候')
  ...
  test('页脚：中英双版版权与许可', async ({ page }) => {
    ...
    await expect(page.locator('.VPFooter')).toContainText('OAS-ISUI contributors')
    await page.goto('/en/', { waitUntil: 'domcontentloaded' })
    await expect(page.locator('.VPFooter')).toContainText('MIT OR Apache-2.0')
  })
})
