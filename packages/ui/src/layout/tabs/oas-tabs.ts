...
import { iconRegistry, type IconName } from '@oas-isui/icons'
...
export class OASTabs extends OASElement {
  ...
  protected override update(): void {
    ...
    this.panels.forEach((panel, idx) => {
      ...
      // slot="label" 自定义标签内容（业界通行的 slot 定制，fallback 到 label 属性纯文本）；
  ...
  private bindScroll(): void {
    ...
    // 滚轮滑动：横向标签栏溢出时滚轮纵向滚动转为横向滑动标签（Chrome/VS Code 标签栏通行）；
   ...
   */
  /**
   * more 模式（业界通行做法）：滚动 + 视口外镜像下拉。tab 全部渲染、tablist 可滚动
  ...
  private startRename(btn: HTMLElement, panel: HTMLElement, value: string): void {
    ...
    // 失焦保存（业界通行 commit on blur：Enter 保存、Esc 取消、blur 保存）；内容未变则静默退出
