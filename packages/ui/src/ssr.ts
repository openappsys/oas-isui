 ...
 *   顶层 `import '@oas-isui/i18n'` 会注入翻译器。
 * - 本入口只导出组件类（从各 oas-*.ts 类文件直接导出，不经过目录 index.ts），
 *   不触碰 DOM API、无任何副作用，可在 Node 环境直接 import，
 *   供高级用户在自有 DOM shim（如 @oas-isui/ssr）里自定义注册/实例化。
 *
 * 用法：
 *   import { OASButton } from '@oas-isui/ui/ssr'
 *   // 自建 DOM shim 时：
 *   customElements.define('oas-button', OASButton)
 *   const btn = document.createElement('oas-button')
 *
 * 组件属性/事件等的类型定义仍从主入口 `@oas-isui/ui` 获取（type-only import 无运行时副作用）。
 */
