/**
 * @oas-isui/nuxt —— Nuxt 3 module。
 *
 * `modules: ['@oas-isui/nuxt']` 引入后自动完成三件事，SSR（DSD 快照）开箱即用：
 *
 ...
 * 2. **theme CSS 注入**：`@oas-isui/theme` 自动追加到 `nuxt.options.css`（DSD 快照
 *    引用的 `--oas-*` token 定义在 theme 的 `:root`，页面必须引入否则 token 回落透明）。
 *    可用 `oasIsui: { theme: false }` 关闭，或传自定义 CSS 入口替换；
 * 3. **SSR helper 自动导入**：`renderOasToString` / `useOasRender`（来自
 *    `@oas-isui/nuxt/ssr`）经 `addImports` 注册，server/api 与 server components
 *    免 import 直接调用；也可显式 `import { renderOasToString } from '@oas-isui/nuxt/ssr'`。
 */
...
export interface ModuleOptions {
  /**
   * 是否自动注入 `@oas-isui/theme` 的全局 CSS（默认 `true`）。
...
export function resolveThemeEntry(theme: boolean | string | undefined): string | null {
  ...
  return typeof theme === 'string' ? theme : '@oas-isui/theme'
}
