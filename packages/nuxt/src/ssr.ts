/**
 * @oas-isui/nuxt SSR helper —— `renderOasToString` / `useOasRender`。
 *
 * `renderToString` 的薄包装：渲染器首次调用时会自行装载 happy-dom DOM shim（进程级副作用，
 * 见 @oas-isui/ssr 的声明）并动态装载白名单组件目录，后续调用仅为渲染本身。locale 经全局
...
import type { RenderToStringOptions } from '@oas-isui/ssr'

export type { RenderToStringOptions } from '@oas-isui/ssr'

/** 渲染选项：内置文案语言（默认沿用当前全局 locale，默认 zh-CN） */
export interface OasRenderOptions extends RenderToStringOptions {}

/**
 * 渲染白名单组件为含 DSD 快照的宿主 HTML 字符串。参数与 `@oas-isui/ssr` 的
 * `renderToString(tag, attrs, slotHTML, opts)` 完全一致，仅做透传包装。
 */
