...
describe('@oas-isui/nuxt module 定义', () => {
  beforeEach(() => {
  ...
  it('module 经 defineNuxtModule 定义：meta.name / configKey / setup 就位', () => {
    ...
    expect(nuxtModule.meta?.name).toBe('@oas-isui/nuxt')
    expect(nuxtModule.meta?.configKey).toBe('oasIsui')
  ...
  it('resolveThemeEntry：默认 true → @oas-isui/theme；字符串 → 自定义入口；false → 不注入', () => {
    expect(resolveThemeEntry(true)).toBe('@oas-isui/theme')
    expect(resolveThemeEntry(undefined)).toBe('@oas-isui/theme')
    expect(resolveThemeEntry('~/assets/oas.css')).toBe('~/assets/oas.css')
    expect(resolveThemeEntry(false)).toBeNull()
  })

  it('oasAutoImports：renderOasToString / useOasRender 指向 @oas-isui/nuxt/ssr', () => {
    expect(oasAutoImports()).toEqual([
      { name: 'renderOasToString', from: '@oas-isui/nuxt/ssr' },
      { name: 'useOasRender', from: '@oas-isui/nuxt/ssr' },
    ])
  })
  ...
  it('setup：theme CSS 注入 nuxt.options.css（去重）', () => {
    ...
    expect(fake.options.css).toEqual(['@oas-isui/theme'])
