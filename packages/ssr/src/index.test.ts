...
import { setLocale } from '@oas-isui/i18n'
 ...
 * 对应组件目录（`@oas-isui/ui/<目录>`，define 副作用注册到 shim 的 customElements）。
 ...
 */
describe('@oas-isui/ssr renderToString', () => {
  beforeEach(() => {
  ...
  it('按需装载：渲染白名单组件后只注册白名单，不装载全量 @oas-isui/ui', async () => {
    ...
    // 非白名单组件未被装载（若误走全量 @oas-isui/ui 入口此处会失败）
