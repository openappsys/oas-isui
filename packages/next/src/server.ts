/**
 * @oas-isui/next RSC 服务端组件 —— `OasComponent`。
 *
 * 用法（Next.js App Router Server Component，须从 `@oas-isui/next/server` 导入）：
 * ```tsx
 ...
 * ```
 *
 * 实现：在服务端调用 `@oas-isui/ssr` 的 `renderToString` 产出
 ...
 *
 * 客户端注册引导见 `@oas-isui/next` 的 `OasRegistry`（"use client" 副作用
 * `import '@oas-isui/ui'`），整个应用的 `oas-*` 元素 upgrade 后由组件接管交互。
