...
/**
 * @oas-isui/next 客户端注册引导单测。
 *
 * 断言 `OasRegistry`（"use client"）的副作用导入 `@oas-isui/ui`
 ...
 */
describe('@oas-isui/next OasRegistry', () => {
  let OasRegistry: (props: { children?: unknown }) => unknown

  beforeAll(async () => {
    // 动态 import：先触发 @oas-isui/ui 副作用注册，再取组件
    const mod = await import('./client.js')
    OasRegistry = mod.OasRegistry as (props: { children?: unknown }) => unknown
  })

  it('副作用注册：import @oas-isui/ui 后 oas-* 组件全局注册（customElements.define）', () => {
