...
OAS-ISUI 的 Next.js（App Router）集成 —— RSC 服务端产 DSD 快照 + 客户端注册引导。
...
#### 1. 服务端：RSC `OasComponent`（`@oas-isui/next/server`）
...
#### 2. 客户端：`OasRegistry` 注册引导（`@oas-isui/next`）

"use client" 副作用 `import '@oas-isui/ui'` 完成全局注册，整个应用的 `oas-*` 元素 upgrade 后由组件接管交互：
...
`renderOas(tag, attrs, slotHTML, opts)`（`@oas-isui/next`）为 `renderToString` 的纯逻辑包装，无 React 依赖，适合自定义包装场景。
...
`@oas-isui/next` — the Next.js (App Router) integration for OAS-ISUI. RSC server-side DSD snapshots + client registration bootstrapping.
...
#### 1. Server: RSC `OasComponent` (`@oas-isui/next/server`)
...
#### 2. Client: `OasRegistry` registration (`@oas-isui/next`)

A "use client" side-effect `import '@oas-isui/ui'` performs global registration. After upgrade, `oas-*` elements across the app are managed by the components:
...
`renderOas(tag, attrs, slotHTML, opts)` (`@oas-isui/next`) is a pure logic wrapper around `renderToString` with no React dependency — suitable for custom wrappers.
