...
// 以当前路由所属 locale 为准（与 @oas-isui/i18n 的 registry 解耦，避免按钮状态与实际页面不一致）
const current = computed<'zh-CN' | 'en'>(() => (lang.value === 'en' ? 'en' : 'zh-CN'))

/** 切换组件内置文案（@oas-isui/i18n），与路由切换并行 */
async function applyI18n(next: 'zh-CN' | 'en'): Promise<void> {
  const { setLocale, registerLocale } = await import('@oas-isui/i18n')
  const pack =
    next === 'en'
      ? (await import('@oas-isui/i18n/en')).default
      : (await import('@oas-isui/i18n/zh-CN')).default
  registerLocale(pack)
  setLocale(next)
}
