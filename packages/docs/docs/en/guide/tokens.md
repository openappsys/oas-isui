...
All of OAS-ISUI's visual language is driven by CSS variables (semantic tokens). Components reference semantic tokens only — **no hardcoded colors, font sizes or spacing** — so theming means overriding CSS variables, without touching components.
