...
> 注意：SSR 快照的 shadow 内联样式引用 `--oas-*` 主题 token（token 定义在 `@oas-isui/theme` 的 `:root`）。页面必须照常引入 `@oas-isui/theme`，否则快照中的组件没有颜色（token 解析失败回落到透明）。
...
- **SSR helper**：`renderOasToString` / `useOasRender` 自动导入（来自 `@oas-isui/nuxt/ssr`），server/api 与 server components 免 import 直接调用；也可显式 `import { renderOasToString } from '@oas-isui/nuxt/ssr'`
