 ...
 *   - 各包发布体积：@oas-isui/ui（dist 总大小 / 浏览器可加载 JS / 全量入口链 / dist/cdn.js）、
 *     @oas-isui/theme（index.css）、@oas-isui/ssr、@oas-isui/i18n、@oas-isui/core、@oas-isui/icons
 *   - 单组件按需引入链：`import '@oas-isui/ui/basic/button'` 等目录入口的"入口 + 依赖"实际加载文件集合
 *     （静态 import 图遍历：入口 index.js → 相对依赖 + @oas-isui/* 包 exports 解析到各自 dist）
 ...
 * - @oas-isui/*：按 node_modules 逐级向上查找包（pnpm workspace 依赖链在
...
// ui 全量入口链：`import '@oas-isui/ui'`（dist/index.js）的实际加载集合
...
const packageRows = [
  ['@oas-isui/ui (dist 总大小)', `${fmtBytes(uiDistTotal)}`, '', ''],
  [
    '@oas-isui/ui (浏览器可加载 JS)',
    `${uiJs.files} 文件`,
    fmtKB(uiJs.rawBytes),
  ...
  ['@oas-isui/ui (dist/cdn.js)', '1 文件', fmtKB(cdn.rawBytes), fmtKB(cdn.gzipBytes)],
  ['@oas-isui/theme (index.css)', '1 文件', fmtKB(theme.rawBytes), fmtKB(theme.gzipBytes)],
  ['@oas-isui/core', `${core.files} 文件`, fmtKB(core.rawBytes), fmtKB(core.gzipBytes)],
  ['@oas-isui/i18n', `${i18n.files} 文件`, fmtKB(i18n.rawBytes), fmtKB(i18n.gzipBytes)],
  ['@oas-isui/icons', `${icons.files} 文件`, fmtKB(icons.rawBytes), fmtKB(icons.gzipBytes)],
  ['@oas-isui/ssr (Node 端)', `${ssr.files} 文件`, fmtKB(ssr.rawBytes), fmtKB(ssr.gzipBytes)],
]
...
const COMPONENT_ENTRIES = [
  { id: 'button', entry: 'basic/button', spec: '@oas-isui/ui/basic/button' },
  { id: 'table', entry: 'data/table', spec: '@oas-isui/ui/data/table' },
  { id: 'form', entry: 'form/form', spec: '@oas-isui/ui/form/form' },
]
...
const BUDGETS = [
  {
    ...
    get: () => cdn.gzipBytes,
    limit: 225 * 1024, // 225 KB（2026-08-14 用户定夺：业界同类[原生 WC 组件库全量]区间 150~300KB 的均值）
    basis: '业界同类（原生 WC 组件库全量单文件）公开区间约 150~300KB 的均值 225KB；v2.0 补齐后实测 gzip 153.2 KB',
  },
  {
    name: '@oas-isui/ui 全量入口链 gzip',
    get: () => fullEntry.gzipBytes,
    limit: 320 * 1024, // 320 KB。实测 271,496 B，上浮约 18%
    basis: '实测 gzip 271,496 B（逐文件求和上界），上浮约 15% 后取整 320 KB',
  },
  {
    name: '@oas-isui/ui/basic/button 链 gzip',
  ...
  {
    name: '@oas-isui/ui/data/table 链 gzip',
...
writeSection('size', {
  ...
  packages: {
    ...
    '@oas-isui/theme': { indexCss: theme },
    '@oas-isui/core': core,
    '@oas-isui/i18n': i18n,
    '@oas-isui/icons': icons,
    '@oas-isui/ssr': ssr,
  },
