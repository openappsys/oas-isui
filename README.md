...
```bash
pnpm add @oas-isui/nuxt   # Nuxt 3 module：isCustomElement + theme 注入 + SSR helper
pnpm add @oas-isui/next   # Next.js：RSC OasComponent + OasRegistry 客户端注册引导
```

## 文档

完整组件文档与 demo：[oasisui.dev](https://oasisui.dev)。

SSR / DSD 指南见文档站 [SSR 页](https://oasisui.dev/guide/ssr)与 `@oas-isui/ssr` 包。
